          ...
          <div class="col-12">
            <div class="card">
              <div class="card-header">
                <form class="row" method="POST" action="<?php echo base_url('admin/muon-sach/tim-kiem/') ?>"> 
                  <div class="col-sm-2">
                    ...
                    <input type="text" name="mamuonsach" class="form-control" placeholder="Mã mượn sách" value="<?php if(isset($post) && (!empty($post['mamuonsach']))) { echo $post['mamuonsach']; } ?>">
                  </div>
                  <div class="col-sm-2">
                    <label>Ngày Mượn</label>
                    <input type="date" name="ngaymuon" class="form-control" placeholder="Ngày mượn" value="<?php if(isset($post) && (!empty($post['ngaymuon']))) { echo $post['ngaymuon']; } ?>">
                  </div>
                  <div class="col-sm-2">
                    <label>Ngày Trả</label>
                    <input type="date" name="ngaytra" class="form-control" value="<?php if(isset($post) && (!empty($post['ngaytra']))) { echo $post['ngaytra']; } ?>">
                  </div>
                  <div class="col-sm-2">
              ...
              <div class="card-footer clearfix">
                <a href="<?php echo base_url('admin/muon-sach/'); ?>" class="btn btn-success">Quay Lại</a>
                <ul class="pagination pagination-sm m-0 float-right">
                	<?php for($i = 1; $i <= $totalPages; $i++){ ?>
                  		<li class="page-item"><a class="page-link" href="<?php echo base_url('admin/muon-sach/tim-kiem/'.$i.'/trang/?mamuonsach='.$post['mamuonsach'].'&ngaymuon='.$post['ngaymuon'].'&ngaytra='.$post['ngaytra']) ?>"><?php echo $i; ?></a></li>
                  	<?php } ?>      
                </ul>
              </div>
            </div>
